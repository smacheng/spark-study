...
public class AppLogSpark {
	...
	public static void main(String[] args) throws Exception {
		...
		JavaRDD<String> accessLogRDD = sc.textFile("G:/data/access.log");
	...
	/**
	 * 根据deviceID进行聚合操作 计算出每个deviceID的总上行流量、总下行流量以及最早访问时间
	 * 
		...
		return accessLogPairRDD.reduceByKey(new Function2<AccessLogInfo, AccessLogInfo, AccessLogInfo>() {
			...
			public AccessLogInfo call(AccessLogInfo accessLogInfo1, AccessLogInfo accessLogInfo2) throws Exception {
				long timestamp = accessLogInfo1.getTimestamp() < accessLogInfo2.getTimestamp()
						? accessLogInfo1.getTimestamp()
						: accessLogInfo2.getTimestamp();
		...
		return aggrAccessLogPairRDD.mapToPair(
				...
				new PairFunction<Tuple2<String, AccessLogInfo>, AccessLogSortKey, String>() {
					...
					public Tuple2<AccessLogSortKey, String> call(Tuple2<String, AccessLogInfo> tuple) throws Exception {
						...
						// 将日志信息封装为二次排序key
						AccessLogSortKey accessLogSortKey = new AccessLogSortKey(accessLogInfo.getUpTraffic(),
								accessLogInfo.getDownTraffic(), accessLogInfo.getTimestamp());
